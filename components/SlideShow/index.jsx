...
function SlideShow({dataType, dataSource}) {
    ...
    const styleClasses = {
        "5 or more": ["video-left-2", "video-left-1", "video-center", "video-right-1", "video-right-2"],
        "3": ["video-left-1", "video-center", "video-right-1"],
        "less than 3 or 4": "Number of slides cannot be less than 3 (makes no sense) or be 4 (asymmetric)."
    }
    ...
    function initImages() {
        if (slideShowRef.current.children.length < 3) {
            setSlideshowImages(dataSource.map((data, index) => {
                return (
                    <div id={index}
                        // style={{border: "solid purple 2px", height: "315px", width: "560px"}}
                         className={`video ${index < 5 ? styleClassToUse[index] : "hidden"}`} key={index}>
    ...
    function initYoutube() {
        if (slideShowRef.current.children.length < 3) {
            setSlideshowVideos(dataSource.map((ytId, index) => {
                return (
                    <div id={index} className={`video ${index < 5 ? styleClassToUse[index] : "hidden"}`} key={index}>
                        <div id={`video-${ytId}`} className="vid-placeholder"
                             onClick={(event) => activateYTVideos(event.currentTarget.id, ytId)}>
                ...
                for (let i = 0; i < children.length; i++) {
                    ...
                    for (let j = 0; j < styleClassToUse.length; j++) {
                        ...
                        if (currentChild.classList.contains(styleClassToUse[j])) {
                            currentChild.classList.remove(styleClassToUse[j]);
                            if (j === 0) {
                                currentChild.classList.add(styleClassToUse[styleClassToUse.length - 1]);
                                break;
                            } else {
                                currentChild.classList.add(styleClassToUse[j - 1]);
                            }
